...
public abstract class AIZombieState : AIState
{
		...
		if (eventType != AITriggerEventType.Exit)
		{
			...
			if (other.CompareTag("Player"))
			{
				...
				if (curType != AITargetType.Visual_Player ||
                    (curType == AITargetType.Visual_Player && distance <_zombieStateMachine.VisualThreat.distance /*_zombieStateMachine.sensorRadius * _zombieStateMachine.sight*/))
				{
                    /*if (distance < _zombieStateMachine.sensorRadius * _zombieStateMachine.sight)
                    {
                    ...
                    RaycastHit hitInfo;
                    if (_zombieStateMachine.attack || ColliderIsVisible(other, out hitInfo, _playerLayerMask))
                    {
                        // Yep...it's close and in our FOV and we have line of sight so store as the current most dangerous threat
                        _zombieStateMachine.VisualThreat.Set(AITargetType.Visual_Player, other, other.transform.position, distance);
                    }
				...
				_zombieStateMachine.satisfaction <= 0.9f && _zombieStateMachine.AudioThreat.type == AITargetType.None)
			{
				...
				if (distanceToThreat < _zombieStateMachine.VisualThreat.distance)
				{
					...
					// AIs sight
					RaycastHit hitInfo;
					if (_zombieStateMachine.attack|| ColliderIsVisible(other, out hitInfo, _visualLayerMask))
					{
	...
	protected virtual bool ColliderIsVisible(Collider other, out RaycastHit hitInfo, int layerMask = -1)
	{
		...
		//Debug.Log("Closest collider is " + hitInfo.transform.name);
	...
	protected virtual bool ColliderIsVisiblePlayer(Collider other, out RaycastHit hitInfo, int layerMask=-1 )
	{
		// Let's make sure we have something to return
		hitInfo = new RaycastHit();
