...
public class FPSController : MonoBehaviour 
{
	...
	protected void Update()
	{
		...
		if (!_jumpButtonPressed && !_isCrouching)
			_jumpButtonPressed = Input.GetKeyDown(KeyCode.Space);
		...
		{
			playerAnim.SetFloat("playerSpeed", -1f);
			_movementStatus = PlayerMoveStatus.NotMoving;
		}
		else
		if (_isCrouching)
		...
		if (_isWalking)
		{
			playerAnim.SetFloat("playerSpeed", 2f);
			_movementStatus = PlayerMoveStatus.Walking;
		}
		else
		{
			playerAnim.SetFloat("playerSpeed", 5f);
	...
	protected void FixedUpdate()
	{
		...
		_moveDirection.x = !_freezeMovement ? desiredMove.x * speed  : 0.0f;
		_moveDirection.z = !_freezeMovement ? desiredMove.z * speed  : 0.0f;
